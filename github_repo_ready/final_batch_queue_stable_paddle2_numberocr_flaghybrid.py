...
KOREAN_OCR_MODEL_NAME = None  # PaddleOCR 2.x 안전 모드
    ...
    for kwargs in attempts:
        try:
            ...
            if log_fn is not None:
                log_fn("OCR 엔진 준비 완료 (PaddleOCR 2.x korean)")
            return engine
...
def create_number_ocr_engine(log_fn=None):
    attempts = [
        {
            "lang": "en",
            "show_log": False,
    ...
    for kwargs in attempts:
        try:
            engine = PaddleOCR(**kwargs)
            if log_fn is not None:
                log_fn("숫자 OCR 엔진 준비 완료 (PaddleOCR 2.x en)")
            return engine
        except Exception as e:
            errors.append(f"{kwargs}: {e}")

    raise RuntimeError("숫자 OCR 엔진 초기화에 실패했습니다. " + " | ".join(errors))
...
def build_crop_variants(crop_img, field_type):
    ...
    scale = 4.0 if field_type == "name" else 4.0
...
def recognize_flag_hybrid(crop_img, ocr_engine):
    ocr_res = recognize_one_crop(crop_img, "flag", ocr_engine)
    cls_res = classify_flag_crop(crop_img)

    ocr_text = (ocr_res.get("text") or "").strip()
    cls_text = (cls_res.get("text") or "").strip()
    ocr_score = float(ocr_res.get("score", 0.0) or 0.0)
    cls_score = float(cls_res.get("score", 0.0) or 0.0)

    # 1) 둘이 일치하면 그대로 사용
    if ocr_text in G_FLAG_VALUES and ocr_text == cls_text:
        return {
            "text": ocr_text,
            "score": max(ocr_score, cls_score),
            "raw_text": f"ocr={ocr_res.get('raw_text','')} | cls={cls_res.get('raw_text','')}",
            "variant": f"hybrid:{ocr_res.get('variant','ocr')}+2class",
        }

    # 2) OCR이 1,000을 충분히 자신있게 내면 우선 채택
    if ocr_text == "1,000" and ocr_score >= 0.55:
        return {
            "text": "1,000",
            "score": ocr_score,
            "raw_text": f"ocr={ocr_res.get('raw_text','')} | cls={cls_res.get('raw_text','')}",
            "variant": f"hybrid:{ocr_res.get('variant','ocr')}",
        }

    # 3) OCR이 비거나 약할 때 2-class가 강하게 1,000이면 채택
    if cls_text == "1,000" and cls_score >= 0.84 and (not ocr_text or ocr_text == "0" or ocr_score < 0.55):
        return {
            "text": "1,000",
            "score": cls_score,
            "raw_text": f"ocr={ocr_res.get('raw_text','')} | cls={cls_res.get('raw_text','')}",
            "variant": "hybrid:2class",
        }

    # 4) OCR이 0을 자신있게 내면 사용
    if ocr_text == "0" and ocr_score >= 0.55:
        return {
            "text": "0",
            "score": ocr_score,
            "raw_text": f"ocr={ocr_res.get('raw_text','')} | cls={cls_res.get('raw_text','')}",
    ...
    # 5) OCR이 None/빈값이면 일단 0으로 채움 (사용자 요청)
    if not ocr_text:
        return {
            "text": "0",
            "score": max(0.50, cls_score * 0.8),
            "raw_text": f"ocr=blank->0 | cls={cls_res.get('raw_text','')}",
            "variant": "hybrid:blank->0",
        }

    # 6) 그 외에는 2-class를 보조로 쓰되 최종값은 0/1,000 중 하나로 강제
    final_text = cls_text if cls_text in G_FLAG_VALUES else "0"
    if final_text not in G_FLAG_VALUES:
        final_text = "0"
...
def detect_columns_from_full_image(image):
    ...
    flag_run = pad_run(flag_run, 0.024, 0.024)
...
class BatchOCRWorker(QThread):
    ...
    def _process_single_image(self, image_path, row_count, ocr_engine, number_ocr_engine):
        ...
        for i in range(row_count):
            ...
            suro_res = recognize_one_crop(suro_rows[i], "number", number_ocr_engine)
            flag_res = recognize_flag_hybrid(flag_rows[i], number_ocr_engine)
    ...
    def run(self):
        try:
            ...
            self.log("OCR 엔진 준비 중... (안전 모드: A=PaddleOCR 2.x korean / F=숫자 전용 PaddleOCR 2.x en / G=전용 0·1,000 판별)")
            ocr_engine = create_korean_ocr_engine(self.log)
            number_ocr_engine = create_number_ocr_engine(self.log)
            ...
            for image_idx, item in enumerate(self.batch_items, start=1):
                ...
                try:
                    image_rows = self._process_single_image(image_path, row_count, ocr_engine, number_ocr_engine)
